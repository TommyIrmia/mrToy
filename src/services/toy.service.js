...
const glabels = ["On wheels", "Car", "Box game", "Art", "Baby", "Doll", "Puzzle", "Outdoor", 'Board game', 'Multi participants', 'Sports', 'Battery Powered']
...
async function query(filterBy = {}) {
    try {
        ...
        if (filter.labels) filter.labels = filter.labels.map(label => label.label)
        const { data } = await axios.get('http://127.0.0.1:3030/api/toy', { params: filter })
        return data
    } catch (err) {
...
async function removeToy(toyId) {
    ...
    try {
        const res = await axios.delete(`http://127.0.0.1:3030/api/toy/${toyId}`, { data: { user } })
        return res
    } catch (err) {
...
async function addToy(toy) {
    const user = userService.getLoggedinUser()
    try {
        const res = await axios.post('http://127.0.0.1:3030/api/toy', { toy, user })
        return res.data
    } catch (err) {
...
async function updateToy(toy, isReview = false) {
    const user = userService.getLoggedinUser()
    try {
        const res = await axios.put('http://127.0.0.1:3030/api/toy', { toy, user, isReview })
        return res.data
    } catch (err) {
